            ...
            if (produits && produits.length > 0) {
                produits.map(function (produit) {
                    ...
                    <div class="card">
                        ...
                        }" class="card-img-top" alt="illustration du produit ${produit.name
                        }">
                        <div class="card-body">
                            <h5 class="card-title">
                            <a href="./produit.html?_id=${produit._id}">${produit.name
                        }</a>
                            </h5>
                            <p class="card-text">${formatPriceToCurrency(
                            parseInt(produit.price)
                        )}</p>
                            <p class="card-text"> ${produit.description}</p>
                            <select id="add-Card-${produit._id
                        }" class="form-select my-4" aria-label="attributes du produit" >`;
                    let i = 0;
                    for (const atr in allAttrs) {
                        templeteProducts += ` <option ${i == 0 ? 'selected' : ''} value="${allAttrs[atr]
                            }">${allAttrs[atr]}</option>`;
                        i++;
                    }
                    //https://stackoverflow.com/questions/26068540/use-onclick-function-after-ajax-call
                    
                    templeteProducts += `</select>
                            <button class="btn btn-primary" onclick="event.preventDefault(); addToCard('${produit._id}', '${produit.name}', '${produit.imageUrl}', '${produit.price}')">Ajouter au panier</button>
                        </div>
                    </div>
                </div>`;
                });
